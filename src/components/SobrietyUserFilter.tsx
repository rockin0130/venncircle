...
interface SobrietyUserFilterProps {
...
const SobrietyUserFilter = ({ selectedUserIds, onSelectionChange }: SobrietyUserFilterProps) => {
  ...
  const filterUsers: FilterUser[] = useMemo(() => {
    ...
    if (isGroup && activeGroup) {
      ...
      seen.add(user?.id || "");
      groups
        .filter((g) => g.shared_pages?.includes("sobriety"))
        .forEach((g) => {
          g.members
...
export default SobrietyUserFilter;
