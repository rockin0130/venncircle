...
const CalendarUserFilter = ({ selectedUserIds, onSelectionChange }: CalendarUserFilterProps) => {
  ...
  // "Everyone" is selected if the sentinel is present OR all individual IDs are selected
  ...
  // Check if a specific user is selected (everyone sentinel means all are selected)
  ...
  const toggleUser = (userId: string) => {
    // If currently "everyone", switch to explicit IDs minus the toggled one
    if (isEveryone) {
    ...
    if (next.has(userId)) {
      ...
      next.add(userId);
      // If all individuals are now selected, switch to everyone sentinel
