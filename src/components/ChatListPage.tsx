...
import { Search, Plus, MoreHorizontal, MessageCircle, X, Check, Loader2 } from "lucide-react";
...
const ChatListPage = ({
  ...
  const { activeFriends, pendingReceived, acceptFriendRequest, declineFriendRequest } = useFriendships();
  ...
  const handleAcceptFriendRequest = async (friendshipId: string) => {
    setFriendActionBusy(friendshipId);
  ...
  return (
    <div className="flex flex-col h-[calc(100svh-5rem)]" style={{ backgroundColor: "#F4F3F0" }}>
      ...
      <div className="flex-1 overflow-y-auto" style={{ WebkitOverflowScrolling: "touch" }}>
        {/* Pinned: pending friend requests */}
        {pendingReceived.length > 0 && (
          <div className="px-4 pt-3 pb-1 space-y-2">
            ...
            {pendingReceived.map((f) => {
              const name = f.friend?.display_name || "Someone";
              const initials = getInitials(name);
              const avatarUrl = f.friend?.avatar_url;
              const busy = friendActionBusy === f.id;
              return (
                <div
                  key={f.id}
                ...
                >
                  <div
                    className="h-9 w-9 rounded-full overflow-hidden flex items-center justify-center shrink-0 bg-primary/15"
                  >
                    {avatarUrl ? (
                      <img src={avatarUrl} alt="" className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-foreground truncate">
                      ...
                      <span className="font-medium text-muted-foreground"> wants to connect</span>
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => void handleAcceptFriendRequest(f.id)}
                    disabled={busy}
                    className="h-7 px-3 rounded-lg bg-primary text-primary-foreground text-[11px] font-semibold disabled:opacity-50 flex items-center gap-1 shrink-0"
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => void handleDeclineFriendRequest(f.id)}
                    disabled={busy}
                    className="h-7 px-3 rounded-lg border border-border text-[11px] font-semibold text-muted-foreground hover:bg-secondary disabled:opacity-50 shrink-0"
                  >
                    Decline
                  </button>
                </div>
