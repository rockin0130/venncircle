...
interface NutritionUserFilterProps {
...
const NutritionUserFilter = ({ selectedUserIds, onSelectionChange }: NutritionUserFilterProps) => {
  ...
  const filterUsers: FilterUser[] = useMemo(() => {
    ...
    if (isGroup && activeGroup) {
      ...
      seen.add(user?.id || "");
      groups
        .filter((g) => g.shared_pages?.includes("nutrition"))
        .forEach((g) => {
          g.members
...
export default NutritionUserFilter;
